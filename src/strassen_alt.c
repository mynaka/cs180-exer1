...
//Multiply matrix A and a transposed matrix B
int** mat_multiply_cached(int** A, int** B, int size){
    int** res = (int**)malloc(size * sizeof(int*));
    for(int i = 0; i < size; i++) res[i] = (int*)malloc(size * sizeof(int)); 

    int** BTransposed = (int**)malloc(size * sizeof(int*));
    for(int i = 0; i < size; i++){
        BTransposed[i] = (int*)malloc(size * sizeof(int));
        for (int j = 0; j < size; j++){
...
int** mul_matrix(int** A, int** B, int size){
    ...
    int** M0 = mat_multiply_cached(add_matrix(A0, A3, newSize), add_matrix(B0, B3, newSize), newSize);
    int** M1 = mat_multiply_cached(add_matrix(A2, A3, newSize), B0, newSize);
    int** M2 = mat_multiply_cached(A0, sub_matrix(B1, B3, newSize), newSize);
    int** M3 = mat_multiply_cached(A3, sub_matrix(B2, B0, newSize), newSize);
    int** M4 = mat_multiply_cached(add_matrix(A0, A1, newSize), B3, newSize);
    int** M5 = mat_multiply_cached(sub_matrix(A2, A0, newSize), add_matrix(B0, B1, newSize), newSize);
    int** M6 = mat_multiply_cached(sub_matrix(A1, A3, newSize), add_matrix(B2, B3, newSize), newSize);
